...
#pragma mark - Funciones del Motor
...
- (void)setupUI {
    UIColor *background = [UIColor colorWithRed:0.02 green:0.02 blue:0.03 alpha:1.0];
    ...
    self.activityIndicator.color = [UIColor colorWithRed:0.95 green:0.08 blue:0.10 alpha:1.0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *identifier = @"XITForgeOptionCell";
    ...
    XITForgeOption *option = self.options[indexPath.row];
    cell.textLabel.text = option.name ?: @"Opción";
    ...
    cell.detailTextLabel.text = option.optionDescription ?: @"";
    cell.detailTextLabel.textColor = [UIColor colorWithWhite:0.56 alpha:1.0];
    cell.detailTextLabel.font = [UIFont systemFontOfSize:13.0];
    cell.detailTextLabel.numberOfLines = 2;
    cell.imageView.image = [UIImage systemImageNamed:@"slider.horizontal.3"];
    cell.imageView.tintColor = [UIColor colorWithRed:0.95 green:0.08 blue:0.10 alpha:1.0];
...
- (NSURL *)realDestinationURLForOption:(XITForgeOption *)option {
    ...
    // Intento 1: MCMFilzaDataContainerPath
    @try {
    ...
    // Intento 2: LSApplicationWorkspace (fallback nativo)
    if (!containerRoot) {
        @try {
    ...
    if (route && route.length > 0) {
        ...
        for (NSString *component in components) {
            ...
            if (![self safePathComponent:component]) return nil;
            destinationFolder = [destinationFolder stringByAppendingPathComponent:component];
        }
    }
    ...
    if (![fm fileExistsAtPath:destinationFolder isDirectory:&isDirectory]) {
        NSError *error = nil;
        BOOL created = [fm createDirectoryAtPath:destinationFolder withIntermediateDirectories:YES attributes:nil error:&error];
        if (!created) {
            NSLog(@"XITFORGE destination directory error: %@", error);
            return nil;
        }
    }
...
- (NSString *)safePathComponent:(NSString *)value {
    ...
    if ([value containsString:@"/"] || [value containsString:@"\\"] || [value containsString:@".."] || [value containsString:@"\0"]) {
        return nil;
    }
    return value;
}
...
- (void)applyOption:(XITForgeOption *)option {
    ...
    NSURL *downloadURL = [NSURL URLWithString:option.fileUrl];
    if (!downloadURL) {
...
- (void)startDownload:(NSURL *)url option:(XITForgeOption *)option destinationURL:(NSURL *)destinationURL {
    [self.activityIndicator startAnimating];
    self.statusLabel.text = [NSString stringWithFormat:@"Descargando %@...", option.name ?: @"archivo"];
    self.statusLabel.hidden = NO;
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    if ([fm fileExistsAtPath:destinationURL.path]) {
        ...
        if (!removed) {
            NSLog(@"XITFORGE remove previous file error: %@", removeError);
            [self showResult:@"No se pudo reemplazar el archivo del juego." success:NO];
            return;
        }
    }
    
    NSError *copyError = nil;
    BOOL copied = [fm copyItemAtURL:location toURL:destinationURL error:&copyError];
    
    if (!copied) {
        NSLog(@"XITFORGE copy file error: %@", copyError);
        [self showResult:@"No se pudo guardar el archivo en el juego." success:NO];
        return;
    }
...
- (void)showResult:(NSString *)message success:(BOOL)success {
    [self.activityIndicator stopAnimating];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:success ? @"✅ Aplicado correctamente" : @"❌ No se pudo aplicar" message:message preferredStyle:UIAlertControllerStyleAlert];
...
- (void)setupUI {
    ...
    self.btnNormal.titleLabel.font = [UIFont boldSystemFontOfSize:14.0];
    [self.btnNormal setTitleColor:white forState:UIControlStateNormal];
    self.btnNormal.titleLabel.numberOfLines = 2;
    self.btnNormal.titleLabel.textAlignment = NSTextAlignmentCenter;
    ...
    self.btnMax.titleLabel.font = [UIFont boldSystemFontOfSize:14.0];
    [self.btnMax setTitleColor:white forState:UIControlStateNormal];
    self.btnMax.titleLabel.numberOfLines = 2;
    self.btnMax.titleLabel.textAlignment = NSTextAlignmentCenter;
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [topGlow.topAnchor constraintEqualToAnchor:self.view.topAnchor constant:-220.0],
        [topGlow.widthAnchor constraintEqualToConstant:340.0],
        [topGlow.heightAnchor constraintEqualToConstant:340.0],
        [bottomGlow.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [bottomGlow.bottomAnchor constraintEqualToAnchor:self.view.bottomAnchor constant:170.0],
        [bottomGlow.widthAnchor constraintEqualToConstant:300.0],
        [bottomGlow.heightAnchor constraintEqualToConstant:300.0],
        [logo.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [logo.topAnchor constraintEqualToAnchor:self.view.safeAreaLayoutGuide.topAnchor constant:40.0],
        [line.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [line.topAnchor constraintEqualToAnchor:logo.bottomAnchor constant:12.0],
        [line.widthAnchor constraintEqualToConstant:60.0],
        [line.heightAnchor constraintEqualToConstant:2.0],
        [pregunta.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [pregunta.topAnchor constraintEqualToAnchor:line.bottomAnchor constant:50.0],
        [pregunta.leadingAnchor constraintGreaterThanOrEqualToAnchor:self.view.leadingAnchor constant:20.0],
        [pregunta.trailingAnchor constraintLessThanOrEqualToAnchor:self.view.trailingAnchor constant:-20.0],
        [self.btnNormal.topAnchor constraintEqualToAnchor:pregunta.bottomAnchor constant:40.0],
        [self.btnNormal.trailingAnchor constraintEqualToAnchor:self.view.centerXAnchor constant:-8.0],
        [self.btnNormal.widthAnchor constraintEqualToConstant:140.0],
        [self.btnNormal.heightAnchor constraintEqualToConstant:80.0],
        [self.btnMax.topAnchor constraintEqualToAnchor:pregunta.bottomAnchor constant:40.0],
        [self.btnMax.leadingAnchor constraintEqualToAnchor:self.view.centerXAnchor constant:8.0],
        [self.btnMax.widthAnchor constraintEqualToConstant:140.0],
        [self.btnMax.heightAnchor constraintEqualToConstant:80.0]
    ]];
}
