...
#pragma mark - XITFORGE Filesystem Engine (FilzaSlop)
static void XITForgeEnsureEngine(void) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        void (*start)(void) = (void (*)(void))dlsym(RTLD_DEFAULT, "MCMFilzaStart");
        if (start) {
            start();
...
static NSString *XITForgeDataContainerPath(NSString *bundleId, NSString **errorOut) {
    ...
    NSString *(*dataPath)(NSString *, NSString **) =
        (NSString *(*)(NSString *, NSString **))dlsym(RTLD_DEFAULT, "MCMFilzaDataContainerPath");
    ...
    NSString *(*virtualRoot)(void) =
        (NSString *(*)(void))dlsym(RTLD_DEFAULT, "MCMFilzaVirtualRoot");
    if (virtualRoot) {
        ...
        if ([root isKindOfClass:[NSString class]] && root.length > 0) {
            ...
            if ([fm fileExistsAtPath:linkPath isDirectory:&isDirectory] && isDirectory) {
                NSLog(@"XITFORGE: usando enlace MCM para %@ -> %@", bundleId, linkPath);
                return linkPath;
            }
        }
    }
    ...
    if (!dataPath) {
        ...
        reason = [NSString stringWithFormat:@"el motor devolvió una ruta no accesible: %@", directPath];
    } else {
        reason = @"el motor no devolvió una ruta para ese bundleId";
    }
    NSLog(@"XITFORGE: contenedor no resuelto para %@: %@", bundleId, reason);
